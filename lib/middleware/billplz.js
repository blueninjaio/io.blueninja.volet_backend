...
const { BillPlzBill, User } = require('../models');

module.exports = {
  validateCallback: async (id, collection_id, paid, state, amount, paid_amount, due_at, email, mobile, name, url, paid_at, x_signature) => {
    const object = { id, collection_id, paid, state, amount, paid_amount, due_at, email, mobile, name, url, paid_at };
    const values = [];
    for (const key in object) {
      values.push(key + object[key]);
    }
    const xsignatureParam = values.sort().join('|');
    const sha256 = HmacSHA256(xsignatureParam, billplz.x_signature_key);
    if (x_signature !== Hex.stringify(sha256)) {
      return false;
    }
    const bill = await BillPlzBill.findOne({ bill_id: id });
    if (!bill) {
      return false;
    }
    const user = await User.findOne({ _id: bill.user });
    if (!user) {
      return false;
    }
    return user;
  ...
  createBill: async (user, amount, description, redirect_url) => {
    let result = await fetch(`${billplz.endpoint}/bills`, {
      ...
      body: JSON.stringify({
        ...
        callback_url: `${local_endpoint}/billplz`,//	Web hook URL to be called after payment’s transaction completed. It will POST a Bill object.
