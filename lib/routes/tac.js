...
const { private_key, transaction_authorization_code } = require('../config');
    ...
    return async (req, res, next) => {
        try {
            ...
            if (e instanceof TokenExpiredError) {
                return res.bad_request('TAC token has expired.');
            }
            next(e);
        }
    };
...
module.exports = {
    ...
    createTacToken: (type, contact, tac_code) => {
        return sign({ type, contact, tac_code }, private_key, {
            expiresIn: transaction_authorization_code.expiry
        });
    }
};