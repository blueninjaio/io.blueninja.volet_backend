...
const BillPlzBill = require('../../models/billplz-bill');
...
module.exports = {
    ...
    createFakePayment: async (req, res) => {
        let { amount, description, redirect_url } = req.body;
        let user = req.user;
        user.credits += amount;
        user.save();
    ...
    billplz_payment: async (req, res) => {
        ...
        let bill = BillPlzBill.findOne({ bill_id: id }).populate('user');
        bill.user.credits += paid_amount;
        bill.user.save();
        console.log(x_signature, sha256, bill);
        return res.ok({x_signature, sha256, bill});
    }
};