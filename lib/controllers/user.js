...
module.exports = {
  ...
  registerUser: async (req) => {
    ...
    const filters = [{ email }, { contact }];
    if (facebook_token) {
      const facebook = await verifyFacebook(facebook_token);
      facebook_id = facebook.id;
      //console.log(facebook);
      filters.push({ facebook_id });
    } else if (google_token) {
      const google = await verifyGoogle(google_token);
      google_id = google.id;
  ...
  loginUser: async (req) => {
    ...
    if (facebook_token) {
      ...
      if (!user) {
        throw new UnauthorizedError('You have not linked your Google account.');
      }
    } else {
  ...
  getUserAgents: async (req) => {
    const users = await User.find({
      account_type: 'UserAgent',
      agent: { is_visible: true }
    }).select('contact f_name l_name photo_base64 gps_coordinates');
    return {
      users: users
    };
  },
  ...
  acceptAgent: async (req) => {
    const { _id } = req.body;
    const user = await User.findOne({ _id });
    if (!user) {
      throw new BadRequestError('Cannot find user by that id.');
    }
    if (user.account_type === 'UserAgent') {
      throw new BadRequestError('They are already a user agent.');
    }
    if (!user.agent.applied) {
      throw new BadRequestError('They do not have a pending application.');
    }
    user.account_type = 'UserAgent';
    user.agent.applied = false;
    await user.save();
  },
  declineAgent: async (req) => {
    const { _id } = req.body;
    const user = await User.findOne({ _id });
    if (!user) {
      throw new BadRequestError('Cannot find user by that id.');
